...
class ProjectManager():
    ...
    def handle_membership(self, msg):
        ...
        if (self.rm_active == False):
            if memberships == {}:
                print("RM: 0 members: ")
            # only passive need to consider primary change
            if ((self.primary == "") or (self.primary not in memberships.keys())):
                self.primary = next(iter(memberships.keys()))
                for server in self.servers:
                    try:
                        self.notify_primary_change(
                            self.primary, (server.ip, server.port))
                    except Exception:
                        pass
        ...
        for mem in memberships:
            if mem not in self.old_membership.keys():
                for server in self.servers:
                    try:
                        self.notify_new_membersip(
                            mem, (server.ip, server.port))
                    except Exception:
                        pass
